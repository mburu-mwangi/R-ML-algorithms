...
require(pacman)
pacman::p_load(pacman,dplyr,GGally,ggplot2,lubridate)
...
#
colnames(advert_data1)
```

```{r}
# Renaming the column male to gender
colnames(advert_data1)[7] <- "Gender"
head(advert_data1)

```
```{r}
# changing the Timestamp column to date time
#
advert_data1$Timestamp <- as.POSIXct(advert_data1$Timestamp)
...
#
gender <- table(advert_data1$Gender)
...
plot(gender01,clicks01,main = "Scatterplot of Gender Vs Clicks on Advert")
...
cor(ages,clicks01)
plot(ages,clicks01,main = "Scatterplot of Age Vs Clicks on ads")

```
```{r}
# Comparing the time spent and clicks on the ads
#
time_spent <- advert_data1$Daily.Time.Spent.on.Site
clicks01 <- advert_data1$Clicked.on.Ad

cor(time_spent,clicks01)
plot(time_spent,clicks01,main = "Scatterplot of Time spent on the site Vs Clicks on ads")
